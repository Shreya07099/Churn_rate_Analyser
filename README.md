...
#Workflow:
1.Identify and prioritize at-risk customers within the highest Customer Lifetime Value (CLV) segments with ≥85% precision, enabling targeted retention campaigns that aim to reduce churn in this segment by 20% within one quarter. This will be done using Gradient boosting methods(XGBoost)
...
4.Build an event-driven AWS pipeline that ingests churn risk scores from the model API, triggers AWS Lambda functions to evaluate rules and personalize messages, and integrates with Amazon Pinpoint or AWS SNS to execute automated, multi-channel re-engagement campaigns via /Email.
#DataSet used:
